...
const RoundTransition = ({
  ...
  const roundScore = Math.round((progressGained / maxScorePerRound) * 100); // Scale to 0-100
  const roundScoreClamped = Math.max(0, Math.min(100, roundScore)); // Ensure 0-100 range
    ...
    if (showJourney) {
      ...
      const timer = setInterval(() => {
        ...
        if (
          ...
          setAnimatedRoundScore(Math.round(currentScore));
        }
      }, duration / steps);
...
//   cumulativeProgress = 0, // Performance-based progress (0-100) - NEW position
//   previousProgress = 0, // Previous round's progress - OLD position
//   isTrainingMode = false, // NEW: flag for training mode
//   mainGameTotalRounds = 36, // NEW: total rounds in main game (for consistent step size)
// }) => {
//   const [countdown, setCountdown] = useState(3);
//   const [showJourney, setShowJourney] = useState(true);
...
//   // Store initial values to prevent re-triggering on prop changes
//   const initialPreviousProgress = useRef(previousProgress);
//   const initialCumulativeProgress = useRef(cumulativeProgress);
...
//   // Calculate per-round score using MAIN GAME step size for consistency
//   const progressGained = cumulativeProgress - previousProgress;
//   const maxScorePerRound = 100 / mainGameTotalRounds; // Always use main game rounds
//   const roundScore = Math.round((progressGained / maxScorePerRound) * 100); // Scale to 0-100
//   const roundScoreClamped = Math.max(0, Math.min(100, roundScore)); // Ensure 0-100 range

//   // Check if robot has passed the goal (only relevant in main game)
//   const hasPassedGoal = !isTrainingMode && journeyProgress >= 75;
...
//   // Journey animation - animate from previousProgress to cumulativeProgress
//   useEffect(() => {
//     if (showJourney && !hasAnimated.current) {
//       hasAnimated.current = true; // Mark as animated to prevent re-runs

//       const duration = 2000; // 2 seconds for the journey animation
//       const steps = 60; // 60 steps for smooth animation
//       const progressDelta =
//         initialCumulativeProgress.current - initialPreviousProgress.current;

//       if (Math.abs(progressDelta) < 0.01 && wasSuccessful) {
//         setJourneyProgress(initialCumulativeProgress.current);
//         setAnimatedRoundScore(roundScoreClamped);
//         const timer = setTimeout(onComplete, 2500);
//         return () => clearTimeout(timer);
//       }
...
//       const increment = progressDelta / steps;
//       const scoreIncrement = roundScoreClamped / steps;
//       let current = initialPreviousProgress.current;
//       let currentScore = 0;
//       let stepCount = 0;
...
//         if (
//           stepCount >= steps ||
//           (progressDelta > 0 && current >= initialCumulativeProgress.current) ||
//           (progressDelta < 0 && current <= initialCumulativeProgress.current)
//         ) {
//           setJourneyProgress(initialCumulativeProgress.current);
//           setAnimatedRoundScore(roundScoreClamped);
...
//       return () => clearInterval(timer);
//     }
//   }, [showJourney, onComplete, roundScoreClamped, wasSuccessful]);
...
//               }}
//             >
//               {isTrainingMode ? "Practice " : ""}Round {roundNumber} Complete
//             </h2>
//             <div
//               style={{
//                 fontSize: "1.25rem",
...
//               {/* Goal marker at 75% - only show in main game */}
//               {!isTrainingMode && (
//                 <g
//                   transform={`translate(${goalPosition.x}, ${goalPosition.y})`}
//                 >
//                   {/* Flag pole */}
//                   <line
//                     x1="0"
